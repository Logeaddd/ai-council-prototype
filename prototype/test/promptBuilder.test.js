...
test("text-only workspace round prompt does not ask the member to propose file_operations", () => {
  ...
  assert.match(messages[0].content, /Do not request .*search_context.*load_context.*extract_archive.*execute_command.*run_code.*install_package/);
...
test("full tool prompt advertises full-only tools while tool tier does not", () => {
  ...
  assert.match(tool[0].content, /extract_archive, execute_command, run_code, and install_package require full permission/);
