...
function toolRequestProtocolLine(options = {}) {
  const tier = options.fileOperationPermissionTier || "text";
  if (tier === "text") return "You have text-only tool permission. Do not request web_search, fetch_url, api_request, list_directory, read_file, search_files, grep_content, search_context, load_context, skill_read, skill_list, skill_search, skill_install, skill_enable, skill_disable, skill_remove, extract_archive, execute_command, process_control, run_code, install_package, run_tests, git_operation, browser_control, database_query, mcp_search_npm, mcp_install_npm, mcp_uninstall, mcp_list_tools, mcp_call, mcp_list_resources, mcp_read_resource, mcp_list_prompts, or mcp_get_prompt; explain what information would be needed.";
