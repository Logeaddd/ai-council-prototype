  ...
  return [
    {
      ...
      content: [
        ...
        "Use tool_requests only when you need the app to fetch a web page, search the web, list/read workspace files, search file names, grep file content, search saved public group history, load a saved public archive round, extract a zip archive, run a shell command, or run a code snippet before you can answer. Do not invent tool results; wait for the app to return them in later context.",
        "Each tool_requests item must include tool and reason. For web_search/search_context include query. For load_context include sessionId and optional round. For fetch_url include url. For list_directory/read_file/extract_archive include path. For extract_archive include destination when you want a specific output folder. For search_files/grep_content include query and optional path. For execute_command include command, optional cwd, optional shell (system, powershell, cmd, bash, sh), optional timeoutMs, and optional background. For run_code include language and code. Allowed tool values: web_search, fetch_url, list_directory, read_file, search_files, grep_content, search_context, load_context, extract_archive, execute_command, run_code.",
...
function toolRequestProtocolLine(options = {}) {
  const tier = options.fileOperationPermissionTier || "text";
  if (tier === "text") return "You have text-only tool permission. Do not request web_search, fetch_url, list_directory, read_file, search_files, grep_content, search_context, load_context, extract_archive, execute_command, or run_code; explain what information would be needed.";
  if (tier === "full") return "You may request built-in tools with tool_requests. Available tools: web_search for live search when configured, fetch_url for reading a public https page, list_directory/read_file for allowed workspace files, search_files for file names, grep_content for file text, search_context for saved public group history, load_context for a saved public archive session or round, extract_archive for zip files inside the group workspace, execute_command for real shell commands in the group workspace, including pipes, redirection, curl | bash, package managers, tests, and background processes, and run_code for real JavaScript/Node, Python, PowerShell, or shell snippets saved and executed inside the group workspace. Tool results will be returned by the app in later context.";
  return "You may request built-in tools with tool_requests. Available tools: web_search for live search when configured, fetch_url for reading a public https page, list_directory/read_file for allowed workspace files, search_files for file names, grep_content for file text, search_context for saved public group history, and load_context for a saved public archive session or round. Tool results will be returned by the app in later context. extract_archive, execute_command, and run_code require full permission.";
}
