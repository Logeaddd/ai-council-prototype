  ...
  return [
    {
      ...
      content: [
        ...
        "Use tool_requests only when you need the app to fetch a web page, search the web, list/read workspace files, search file names, grep file content, search saved public group history, load a saved public archive round, extract a zip archive, or run a shell command before you can answer. Do not invent tool results; wait for the app to return them in later context.",
        "Each tool_requests item must include tool and reason. For web_search/search_context include query. For load_context include sessionId and optional round. For fetch_url include url. For list_directory/read_file/extract_archive include path. For extract_archive include destination when you want a specific output folder. For search_files/grep_content include query and optional path. For execute_command include command, optional cwd, optional shell (system, powershell, cmd, bash, sh), optional timeoutMs, and optional background. Allowed tool values: web_search, fetch_url, list_directory, read_file, search_files, grep_content, search_context, load_context, extract_archive, execute_command.",
