  ...
  return [
    {
      ...
      content: [
        ...
        "Use tool_requests only when you need the app to fetch a web page, search the web, call an HTTP API, list/read workspace files, search file names, grep file content, search saved public group history, load a saved public archive round, extract a zip archive, run a shell command, run a code snippet, install a package, run tests, perform Git work, control a real browser, query a SQLite database, search for installable MCP tools, install or remove an external MCP server, list or call configured external MCP tools, read configured external MCP resources, or get configured external MCP prompts before you can answer. Do not invent tool results; wait for the app to return them in later context.",
        "Each tool_requests item must include tool and reason. For web_search/search_context/mcp_search_npm include query. For load_context include sessionId and optional round. For fetch_url/api_request/browser_control include url. For api_request include method and optional headers, json, or body. For list_directory/read_file/extract_archive/database_query include path. For database_query include sql and optional params, mode (query or execute), create, and maxRows. For extract_archive include destination when you want a specific output folder. For search_files/grep_content include query and optional path. For execute_command include command, optional cwd, optional shell (system, powershell, cmd, bash, sh), optional timeoutMs, and optional background. For run_code include language and code. For install_package include manager (npm, pip, cargo, go, or gem) and packageName. For run_tests include runner (npm, pytest, cargo, or custom), optional cwd, and command for custom. For git_operation include action (status, init, branch, create_branch, switch_branch, commit, pull, push), optional branch, remote, message, paths, cwd, and timeoutMs. For browser_control include optional steps with action open, navigate, click, type, evaluate, screenshot, wait, or wait_for_selector. For mcp_install_npm include packageSpec or catalogId, optional serverId, binName, and args. For mcp_uninstall include serverId. For mcp_list_tools, mcp_list_resources, and mcp_list_prompts include optional serverId. For mcp_call include mcpToolName and arguments; include serverId only after choosing a specific server or when the same tool name appears on more than one MCP server. For mcp_read_resource include serverId when needed and uri. For mcp_get_prompt include serverId when needed, promptName, and optional arguments. Allowed tool values: web_search, fetch_url, api_request, list_directory, read_file, search_files, grep_content, search_context, load_context, extract_archive, execute_command, run_code, install_package, run_tests, git_operation, browser_control, database_query, mcp_search_npm, mcp_install_npm, mcp_uninstall, mcp_list_tools, mcp_call, mcp_list_resources, mcp_read_resource, mcp_list_prompts, mcp_get_prompt.",
...
function toolRequestProtocolLine(options = {}) {
  const tier = options.fileOperationPermissionTier || "text";
  if (tier === "text") return "You have text-only tool permission. Do not request web_search, fetch_url, api_request, list_directory, read_file, search_files, grep_content, search_context, load_context, extract_archive, execute_command, run_code, install_package, run_tests, git_operation, browser_control, database_query, mcp_search_npm, mcp_install_npm, mcp_uninstall, mcp_list_tools, mcp_call, mcp_list_resources, mcp_read_resource, mcp_list_prompts, or mcp_get_prompt; explain what information would be needed.";
  if (tier === "full") return "You may request built-in tools with tool_requests. Available tools: web_search for live web search, fetch_url for reading a public https page, api_request for real HTTP API calls with method, headers, json, or body, list_directory/read_file for allowed workspace files, search_files for file names, grep_content for file text, search_context for saved public group history, load_context for a saved public archive session or round, extract_archive for zip files inside the group workspace, execute_command for real shell commands in the group workspace, including pipes, redirection, curl | bash, package managers, tests, and background processes, run_code for real JavaScript/Node, Python, PowerShell, or shell snippets saved and executed inside the group workspace, install_package for real npm, pip, cargo, go, or gem installs into managed group-workspace environments, run_tests for real npm, pytest, cargo, or custom test commands, git_operation for real Git status, init, branch, switch, commit, pull, and push inside the group workspace, browser_control for opening a real browser page, clicking, typing, evaluating page state, and saving screenshots, database_query for reading or writing SQLite databases inside the group workspace, mcp_search_npm for real npm registry search before adding MCP tools, mcp_install_npm and mcp_uninstall for managed npm MCP servers, mcp_list_tools and mcp_call for configured external MCP tools, mcp_list_resources and mcp_read_resource for configured external MCP resources, and mcp_list_prompts and mcp_get_prompt for configured external MCP prompts. mcp_call can infer the server when the tool name is unique; include serverId for ambiguous tool names. Tool results will be returned by the app in later context.";
  return "You may request built-in tools with tool_requests. Available tools: web_search for live web search, fetch_url for reading a public https page, api_request for real HTTP API calls, list_directory/read_file for allowed workspace files, search_files for file names, grep_content for file text, search_context for saved public group history, load_context for a saved public archive session or round, and database_query for read-only SQLite SELECT queries inside the group workspace. Tool results will be returned by the app in later context. extract_archive, execute_command, run_code, install_package, run_tests, git_operation, browser_control, mcp_search_npm, mcp_install_npm, mcp_uninstall, mcp_list_tools, mcp_call, mcp_list_resources, mcp_read_resource, mcp_list_prompts, and mcp_get_prompt require full permission. database_query write operations require full permission.";
}
