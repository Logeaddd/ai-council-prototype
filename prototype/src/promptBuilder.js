  ...
  return [
    {
      ...
      content: [
        ...
        "If speaking, use keys: status, position, argument, objections, objection_items, resolved_ids, suggested_revision, artifacts, file_operations, tool_requests, task_contract, task_delegations, delegation_handoff, confidence, memory_candidates, context_invalidations.",
        "When the context labels you [Task intake owner], call the native record_task_contract tool exactly once before delegating or repeating work. Its taskContract uses mode, objective, requiresWorkspace, requiresVerification, deliverables, artifacts, completionCriteria, and nextAction. For each requested workspace file, artifacts must state its path or extension and any structural requirements such as minimumPages or requiresImages. Its mode is delivery when the user wants work performed or an outcome produced, or discussion when they want analysis, advice, or an answer without carrying out work. Also include collaboration with required, beforeFirstMutation, minimumDelegations, types, and reason when the user explicitly requires work to be shared, delegated, researched by another member, reviewed by another member, or handed off between members. Set collaboration.required false when the user did not require a collaboration step; do not infer it merely because several members exist. requiresWorkspace means the task needs a workspace mutation, build, package, or other local output; merely reading a workspace file does not make it true. Decide from the user's actual meaning in any language; do not classify by keywords. A delivery contract does not prove completion and does not replace real tool or artifact evidence.",
        "Only an [Execution owner] may create bounded sub-work. Use the native delegate_task tool for every new delegation; do not merely say in prose that you will delegate. It requires delegationType (research, implementation, review, or unblocker), assigneeId, task, expectedEvidence, allowedTools, allowWorkspaceMutation, and allowedPaths when workspace mutation is allowed. An unblocker may instead use allowRuntimeMutation for managed package, runtime, Skill, or MCP acquisition only; it never permits arbitrary commands or project-output writes. The task_delegations JSON field remains only for older persisted-provider compatibility. Delegate a narrow fact, repair, review, or unblocker slice, never the entire user request or final delivery ownership. A contributor must return delegation_handoff with the exact delegation_id, a concise summary, and concrete evidence; do not claim another member's work or independently declare the whole task complete.",
        ...
        "Context replacement is a separate, source-specific decision. When the current user instruction actually replaces, cancels, or corrects a visible retained instruction, include context_invalidations with {source:{type,id},reason}. Choose source only from the exact Context source references supplied in this prompt. Do not infer a replacement merely from topic overlap, wording changes, or recency; do not invalidate evidence, a whole session, or a source whose meaning you cannot establish. The app will reject invented, hidden, or current-turn source references and will record accepted replacements durably. Use an empty array when no exact retained instruction has been replaced. This is semantic interpretation of the user's meaning in any language, not keyword matching.",
