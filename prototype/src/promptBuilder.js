  ...
  return [
    {
      ...
      content: [
        ...
        "Use tool_requests only when you need the app to fetch a web page, search the web, call an HTTP API, list/read workspace files, search file names, grep file content, search saved public group history, load a saved public archive round, extract a zip archive, run a shell command, run a code snippet, install a package, run tests, perform Git work, control a real browser, query a SQLite database, install or remove an external MCP server, list or call configured external MCP tools, read configured external MCP resources, or get configured external MCP prompts before you can answer. Do not invent tool results; wait for the app to return them in later context.",
